...
import { useGetCaseStaticCheckedItemsQuery,useCreateCarModelInfoMutation } from 'generated/graphql';
...
export const AddNewCarModelModal: React.FC<AddNewCarModel> = (props) => {

    const [add] = useCreateCarModelInfoMutation();
                ...
                return <>
                    <Select
                        ...
                        onChange={(value) => {
                            ...
                            item[0].parts = value;
                            // _partList[_value.id].parts = value;
    ...
    }, [optionHook.data?.getCaseStaticCheckedItems?.carSpareParts])
    ...
    return <> <Modal size='l' visible={props.isShow} caption={<Localized id='compliance-addNewCar'></Localized>} onClose={() => { props.setIsShowAddDialog(false) }}>
        <Modal.Body>
            <Form>
                <Form.Item label={'车型'} status={carModuleNameSt}>
                ...
                <Form.Item label={'零部件'} showStatusIcon={false}>
                    <Table
                        bordered
                        recordKey={'id'}
                        records={partsList}
