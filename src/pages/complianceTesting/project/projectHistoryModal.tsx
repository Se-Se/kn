...
export const ProjectHistoryModal: React.FC<ProjectHistoryModalType> = (props) => {
    ...
    const modifyListDataHook = useChangeProjectHistoryQuery({
        variables: {
            teamId: teamId, projectId: projectId, search:pageQueryInfo
        }
    })
    ...
    useEffect(() => {
        setModifyListDataHook(modifyListDataHook.data?.changeProjectHistory?.history || []);
        setProjectListDataCount(modifyListDataHook.data?.changeProjectHistory?.count || 0);
