...
import { autoPartsSystemListGroup, autoPartsRelieveRelationSystem, autoSystemDetail, autoPartsRelationSystem } from "../../../../util/componentApi";
...
export const SystemList: React.FC<Props> = (props: Props) => {
    ...
    const jumpToDetail = (id: string) => {
        ...
        w.location.href = `/lingshu/assetsManage/systemManage/systemDetail/${Number(id)}`;
    }
    ...
    const getDrawerList = () => {
        ...
        autoPartsSystemListGroup(request).then((res: any) => {
            if (res?.code === 0) {
                const { OperatingSystem, SelectedIdList } = res;
                formatterOptList(OperatingSystem || []);
