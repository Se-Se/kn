...
import { autoPartsImportData } from '../../../util/componentApi';
import { staticResourceDownloadTemplate } from '../../../util/api';
...
export const UploadDrawer: React.FC<DrawerProps> = (props: DrawerProps) => {
  ...
  const [relation, setRelation] = useState<any>(null);//所需车型
  ...
  const onSubmit = (e: any) => {
    ...
    if (e.validateResult === true) {
      let request: any = {
        belongIds: [Number(relation)],
        uploadFileId: successId,
        subModule: props.tabV
      };

      autoPartsImportData(request).then((res: any) => {
        if (res?.code === 0) {
          if (res.hasError) {
  ...
  const handleDown = () => {
    ...
    let name: any = `autoParts_template.xls`;
  ...
  return (
    <Style>
      <Form labelAlign='left' ref={formRef} labelWidth={120} onSubmit={onSubmit}>
        ...
        >
          <div className={`kn-upload ${uploadStep === -1 ? 'error' : null}`} style={{ display: 'flex' }}>
            <FormItem name='fileName' className={`${uploadStep === -1 ? 'kn-error' : ''}`} requiredMark={true} style={{ width: 336 }} label='导入文件' rules={[{ required: true, type: 'error' }]}>
          ...
          <div >
            <MyCasCader placeholder='请选择' label="所属车型" statusIcon={false} isSaved={isSaved} required={false} visible={props.visible} onChange={(value: any) => handleCasCaderChange(value)} optList={props.optList} value={relation}></MyCasCader>
          </div>
        </Drawer>
      </Form>
