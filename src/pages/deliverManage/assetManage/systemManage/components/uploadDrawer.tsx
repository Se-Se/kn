...
import { systemImportData, staticResourceDownloadTemplate } from '../../../util/api';
...
export const UploadDrawer: React.FC<DrawerProps> = (props: DrawerProps) => {
  ...
  const onSubmit = (e: any) => {
    ...
    if (e.validateResult === true) {
      ...
      systemImportData(request).then((res: any) => {
        if (res?.code === 0) {
          if (res.hasError) {
  ...
  const handleDown = () => {
    ...
    let name: any = `system_${props.subModule}_template.xls`;
    ...
    <Style>
      <Form labelAlign='left' ref={formRef} labelWidth={120} onSubmit={onSubmit}>
        ...
        >
          ...
          <CasCaderContent>
            <MyCasCader isSaved={isSaved} required={false} visible={props.visible} onChange={(value: any) => setParts(value)} optList={props.optList} value={parts}></MyCasCader>
          </CasCaderContent>
        </Drawer>
      </Form>
    </Style>
