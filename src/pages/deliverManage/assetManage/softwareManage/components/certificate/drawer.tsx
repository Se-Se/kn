...
export const MyDrawer: React.FC<DrawerProps> = (props: DrawerProps) => {
  ...
  const onSubmit = (e) => {
    ...
    if(belongSystemIds.length==0){
      ...
      data.certificateFileId=successId
      if (props.id) {
        let editRequest: any = { ...data };
        editRequest.id = props.id
        editSoftWatre(data).then((res:any)=>{
          if(res.code === 0 ){
            setLoading(false)
        ...
        createSoftware(data).then((res:any)=>{
          if(res.code === 0 ){
            setLoading(false)
  ...
  const  fetchData = () => {
    getSoftWareEcho({Module:'certificate',softwareId:props.id}).then((res:any)=>{
      if(res.code === 0){
        console.log(res)
      ...
      <Form labelAlign='left' ref={formRef} labelWidth={120} onSubmit={onSubmit} statusIcon={true}>
        ...
        >
          <FormContentTop>
            ...
            <FormItem name='certificatePath' requiredMark={true} label='存放路径' style={{marginTop:20 }} rules={[{ required: true, message: '请输入存放路径', type: 'error' }]}>
              <Input placeholder='请输入' maxlength={20}></Input>
            </FormItem>
            <FormItem name='certificatePurpose' requiredMark={true} label='用途' style={{marginTop:20,marginLeft:10 }} labelWidth={110}>
              <Input placeholder='请输入' maxlength={20}></Input>
            </FormItem>
