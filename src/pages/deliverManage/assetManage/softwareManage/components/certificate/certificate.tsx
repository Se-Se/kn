...
import { CertificateObj, ModelList, SoftWareProps } from "../type";
...
for (let i = 0; i < 50; i++) {
  data.push({
    ...
    needed: 'Y',
    description: '数据源',
    firmPackage:(i%2==0)?true:false,
    detail: {
      name: '嵌套信息读取',
    },
  });
}
export const Certificate: React.FC<SoftWareProps> = (props:SoftWareProps) => {
  ...
  const fetchData=async ()=>{
    ...
    setTableLoading(true)
     const resp = await getModuleList(params)as ModelList<CertificateObj>
      if(resp?.code === 0){
  ...
  const jumpToDetail=(id:string)=>{
      history.push(`/lingshu/assetsManage/softwareManage/certificateDetail/${Number(id)}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      minWidth:200,
      colKey: 'certificateFileUrl',
      title: '文件',
      sorter:true,
      //@ts-ignore
      render(context){
        const text = context.row.certificateFileUrl ? (
          <span>
            已上传
...
// 删除
  ...
  return (
    <Style>
      ...
      <MyDrawer optList={props.relationSystemGroupOptions}editData={editData} visible={drawerVisible} id={drawerShowId} save={() => save()} onClose={(isFetch) => drawerOnClose(isFetch)}></MyDrawer>;
      <UploadDrawer optList={props.relationSystemOptions} visible={uploadDrawerVisible} onClose={() => setUploadDrawerVisible(false)} type='certificate'></UploadDrawer>
      {batchModal()}
