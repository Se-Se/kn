...
import { deleteSoftwareApi, getModuleList, relationSystem, softwaregetGetRetrieveList } from "pages/deliverManage/util/softwareApi/api";
import { FirmListObj, ModelList, retrieveListType, SoftWareProps } from "../type";
...
const SelectTitle = style.div`
  ...
  min-width:56px;
`
export const FirmWare: React.FC<SoftWareProps> = (props: SoftWareProps) => {
  ...
  const fetchData = async () => {
    ...
    const resp = await getModuleList(params) as ModelList<FirmListObj>
    if (resp?.code === 0) {
  ...
  const jumpToDetail = (id: string) => {
    history.push(`/lingshu/assetsManage/softwareManage/firmDetail/${Number(id)}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      minWidth: 200,
      colKey: 'firmwarePackageFileUrl',
      title: '固件包',
      sorter: true,
      render(cell: any) {
        if (cell.row.firmwarePackageFileUrl) {
          return (
            <ProgressTag progress={1} />
          )
        } else {
          return (
            <ProgressTag progress={0} />
          )
        }
    ...
    },
    {
      ...
      width: 150,
      minWidth: 150,
      colKey: 'firmwareReleaseTimeFormat',
      title: '发布时间',
      sorter: true
    },
    {
  ...
  return (
    <Style>
      ...
      <MyDrawer optList={props.relationSystemGroupOptions} editData={editData} visible={drawerVisible} id={drawerShowId} onClose={(isfetch) => drawerOnClose(isfetch)}></MyDrawer>;
      <UploadDrawer optList={props.relationSystemOptions} visible={uploadDrawerVisible} onClose={() => setUploadDrawerVisible(false)} type='firmWare'></UploadDrawer>
      {batchModal()}
