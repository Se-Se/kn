...
export const Page: React.FC = () => {
  ...
  // 获取详情信息
  const fetch = () => {
    getSoftwareDetail({softwareId:Number(params.firmId)}).then((res:any)=>{
      if(res.code === 0){
        const data = res.result
        data.moduleString = res.moduleString
        setFileName(res.result.firmwarePackageFileName)
                  ...
                  <DetailRow>
                    <Row>
                      ...
                      <Col span={6}>
                        <Row>
                          <Col span={2}>
                            <DetaiLabel>发布时间</DetaiLabel>
                          </Col>
                          <Col span={4}>
                            <span>{detailData.firmwareReleaseTimeFormat}</span>
                          </Col>
                        </Row>
                      </Col>
                    </Row>
                  ...
                  <DetailRow>
                    <Row>
                      <Col span={6}>
                        <Row>
                          <Col span={2}>
                            <DetaiLabel>版本</DetaiLabel>
                          </Col>
                          <Col span={2}>
                            <span>{detailData.firmwareVersion}</span>
                          </Col>
                        </Row>
                      </Col>
                      <Col span={6}>
              ...
              <TableContent>
                <Tabs placement={"top"} size={"medium"} defaultValue={"1"}>
                  <TabPanel value="1" label="详细信息">
                    <MyUpload softWareId={Number(params.firmId)}></MyUpload>
                  </TabPanel>
                  <TabPanel value="2" label="系统信息">
                    <SystemDataTable softWareId={Number(params.firmId)}></SystemDataTable>
