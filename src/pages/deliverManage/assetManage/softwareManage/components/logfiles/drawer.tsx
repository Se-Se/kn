...
export const MyDrawer: React.FC<DrawerProps> = (props: DrawerProps) => {
  ...
  const fetchData = () => {
    getSoftWareEcho({Module:'log',softwareId:props.id}).then((res:any)=>{
      if(res.code === 0){
        console.log(res)
        const data = res.result
        let formData :any = {
          name:data.name,
          logTypeId:data.logType,
          logPath:data.logPath,
          logPurpose:data.logPurpose,
      ...
      <Form labelAlign='left' ref={formRef} labelWidth={120} onSubmit={onSubmit} statusIcon={true}>
        ...
        >
          <FormContentTop>
            ...
            {/* <FormItem name='system' requiredMark={true} label='所属系统' rules={[{ required: true, message: '请选择所属系统', type: 'error' }]}>
            ...
            <FormItem name='logTypeId' requiredMark={true} label='类型' style={{marginTop:20,marginLeft:10 }} labelWidth={110} >
              <Select>
                {props.logTypeSelector.map((item,index)=>
                <Option value ={item.value} key={index}>{item.label}</Option>
                )}
              </Select>
            </FormItem>
            <FormItem name='logPath' requiredMark={true} label='路径' style={{marginTop:20 }} rules={[{ required: true, message: '请输入路径', type: 'error' }]}>
              <Input placeholder='请输入' maxlength={20}></Input>
            </FormItem>
            <FormItem name='logPurpose' requiredMark={true} label='用途' style={{marginTop:20,marginLeft:10 }} labelWidth={110} >
              <Input placeholder='请输入' maxlength={20}></Input>
            </FormItem>
