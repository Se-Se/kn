...
import { LogObj, LogsFileProps, ModelList, retrieveListType, SoftWareProps } from "../type";
...
const SelectTitle = style.div`
  ...
  min-width:56px;
`
export const LogsFile: React.FC<LogsFileProps> = (props:LogsFileProps) => {
  ...
  const fetchData= async ()=>{
    ...
    const resp = await getModuleList(params) as ModelList<LogObj>
      if(resp?.code === 0){
  ...
  const jumpToDetail=(id:string)=>{
      history.push(`/lingshu/assetsManage/softwareManage/logsFileDetail/${Number(id)}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      //@ts-ignore
      render(context){
        const text = context.row.logFileUrl ? (
          <span>
            已上传
...
// 删除
  ...
  return (
    <Style>
      ...
      <MyDrawer optList={props.relationSystemGroupOptions}  visible={drawerVisible} id={drawerShowId}  onClose={(isFetch) => drawerOnClose(isFetch)} logTypeSelector={props.logTypeSelector}></MyDrawer>;
      <UploadDrawer optList={props.relationSystemOptions} visible={uploadDrawerVisible} onClose={() => setUploadDrawerVisible(false)} type='log'></UploadDrawer>
      {batchModal()}
