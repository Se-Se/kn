...
import { ModelList, retrieveListType, SecretKeyObj, SoftWareProps } from "../type";
...
const SelectTitle = style.div`
  ...
  min-width:56px;
`
export const Encryptionkey: React.FC<SoftWareProps> = (props:SoftWareProps) => {
  ...
  const fetchData= async()=>{
    ...
    const resp =await getModuleList(params) as ModelList<SecretKeyObj>
      if(resp?.code === 0){
  ...
  const jumpToDetail=(id:string)=>{
      history.push(`/lingshu/assetsManage/softwareManage/encryptionkeyDetail/${Number(id)}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      minWidth:200,
      colKey: 'secretKeyFileUrl',
      title: '文件',
      sorter:true,
      //@ts-ignore
      render(context){
        const text = context.row.secretKeyFileUrl ? (
          <span>
            已上传
  ...
  return (
    <Style>
      ...
      <UploadDrawer optList={props.relationSystemOptions} visible={uploadDrawerVisible} onClose={() => setUploadDrawerVisible(false)} type='secretKey'></UploadDrawer>
      {batchModal()}
