...
export const MyDrawer: React.FC<DrawerProps> = (props: DrawerProps) => {
  ...
  const fetchData = () => {
    getSoftWareEcho({Module:'secretKey',softwareId:props.id}).then((res:any)=>{
      if(res.code === 0){
        console.log(res)
        const data = res.result
        let formData :any = {
          name:data.name,
          secretKeyPath:data.secretKeyPath,
          secretKeyPurpose:data.secretKeyPurpose,
          remark:data.remark,
          fileName:data.secretKeyFileName
        }
      ...
      <Form labelAlign='left' ref={formRef} labelWidth={120} onSubmit={onSubmit} statusIcon={true}>
        ...
        >
          <FormContentTop>
            ...
            {/* <FormItem name='system' requiredMark={true} label='所属系统' rules={[{ required: true, message: '选择所属系统', type: 'error' }]}>
            ...
            <FormItem name='secretKeyPath' requiredMark={true} label='存放路径' style={{marginTop:20 }} rules={[{ required: true, message: '请输入存放路径', type: 'error' }]}>
              <Input placeholder='请输入' maxlength={20}></Input>
            </FormItem>
            <FormItem name='secretKeyPurpose' requiredMark={true} label='用途'  style={{marginTop:20,marginLeft:10 }} labelWidth={110}>
              <Input placeholder='请输入' maxlength={20}></Input>
            </FormItem>
            ...
            <div style={{display:'flex',alignItems:'baseline'}}>
              <FormItem name='fileName' requiredMark={true} label='文件'labelWidth={110} style={{ paddingLeft: 10 }}>
            ...
            {/* {uploadStep !== 0 ? <a className='delete_text' onClick={() => handleDelete()} style={{marginLeft:10}}>删除</a> : null} */}
            </div>
            {uploadStep===-1&&(
            <div className='t-is-error' style={{marginLeft:120,marginTop:-26}}>
