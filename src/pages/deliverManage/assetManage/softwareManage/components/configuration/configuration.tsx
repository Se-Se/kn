...
import { ConfigObj, ConfigProps, ModelList, retrieveListType, SoftWareProps } from "../type";
...
export const Configuration: React.FC<ConfigProps> = (props:ConfigProps) => {
  ...
  const fetchData= async()=>{
    ...
    const resp = await getModuleList(params) as ModelList<ConfigObj>  
      if(resp?.code === 0){
  ...
  const jumpToDetail=(id:string)=>{
      history.push(`/lingshu/assetsManage/softwareManage/configDetail/${Number(id)}`);
...
// 删除
  ...
  return (
    <Style>
      ...
      <MyDrawer optList={props.relationSystemGroupOptions} visible={drawerVisible} id={drawerShowId}  onClose={(isFetch) =>drawerOnClose(isFetch)} configTypeSelector={props.configTypeSelector}></MyDrawer>;
      <UploadDrawer optList={props.relationSystemOptions} visible={uploadDrawerVisible} onClose={() => setUploadDrawerVisible(false)} type='config'></UploadDrawer>
      {batchModal()}
